    ...
    if (Token) {
        ...
        Client.findOne({ _id: req.body.client_id }, (error, client) => {
            ...
            if (client) {
                ...
                res.status(400).json({ sucess: false, message: 'Invalid client' })
            }
        })
    } else return res.status(403).json({ sucesss: false, message: 'Unauthorized' });
}
...
api.getAll = (User, Budget, Token) => (req, res) => {
    ...
    } else return res.state(403).send({ success: false, message: 'Unauthorized' });
}
...
api.index = (User, Budget, Client, Token) => (req, res) => {
    ...
    } else return res.state(401).send({ success: false, message: 'Unauthorized'});
}

  ...
  <main class="l-home-page">
    ...
    <div class="l-home">
      ...
      <list v-if="listPage">
        ...
        <list-body slot="list-body" :budgetsVisible="budgetsVisible" :data="budgetsVisible ? budgets : clients"></list-body>
      </list>
    </div>

    <v-snackbar :timeout="timeout" bottom="bottom" color="red lighten-1" v-model="snackbar">
      {{ message }}
    </v-snackbar>
  ...
  methods: {
    getAllBudgets () {
      Axios.get(`${BudgetManagerAPI}/api/v1/budget`, {
        ...
        params: { client_id: this.$cookie.get('user_id') }
      }).then(({data}) => {
